...
public final class FindMeetingQuery {
  public Collection<TimeRange> query(Collection<Event> events, MeetingRequest request) {
    ...
    // If requested duration is longer than one day, 
    // return no options as a valid time range.
    ...
    // If no mandatory or optional attendees are requested,
    // return the whole day as a valid time range.
    if (attendees.size() == 0 && optionalAttendees.size() == 0) {
      return Arrays.asList(TimeRange.WHOLE_DAY);
    }

    // If only mandatory attendees are requested, 
    // return available time ranges for mandatory attendees.
    if (optionalAttendees.size() == 0) {
      ArrayList<TimeRange> availableTimeRanges = 
          returnAvailableTimeRanges(events, attendees, requestDuration);
      return availableTimeRanges;
    }

    // If only optional attendees are requested, 
    // return available time ranges for optional attendees.
    if (attendees.size() == 0) {
      ArrayList<TimeRange> availableTimeRanges = 
          returnAvailableTimeRanges(events, optionalAttendees, requestDuration);
      return availableTimeRanges;
    }
    ...
    ArrayList<TimeRange> optionalAvailableTimeRanges = 
        returnAvailableTimeRanges(events, optionalAttendees, requestDuration);
    ArrayList<TimeRange> mutualAvailableTimeRanges = 
        getMutualAvailableTimeRanges(mandatoryAvailableTimeRanges, 
        optionalAvailableTimeRanges, requestDuration);
  ...
  private ArrayList<TimeRange> 
      sortTimeRanges(ArrayList<TimeRange> busySortedTimeRanges) {
    Collections.sort(busySortedTimeRanges, TimeRange.ORDER_BY_START);
    return busySortedTimeRanges;
  }

  // Returns an ArrayList of TimeRanges that attendees are busy for.
  // Sorted by start time in ascending order.
  private ArrayList<TimeRange> getSortedBusyTimeRanges(Collection<Event> events, 
  ...
  private TimeRange merge(TimeRange firstTimeRange, TimeRange secondTimeRange) {
    // Set start equal to start of firstTimeRange
    // since the Time Ranges are ordered by start time.
  ...
  // Returns ArrayList of TimeRanges with 
  // merged overlaps that attendees are busy for. 
  private ArrayList<TimeRange> 
      getMergedBusyTimeRanges(ArrayList<TimeRange> sortedBusyTimeRanges) {
  ...
  private ArrayList<TimeRange> 
      getAvailableTimeRanges(ArrayList<TimeRange> mergedBusyTimeRanges,
      int requestDuration) {
  ...
  private TimeRange getOverlappingTimeRange(TimeRange mandatoryTimeRange,
  ...
  // Returns available time ranges for one type of attendee
  private ArrayList<TimeRange> 
      returnAvailableTimeRanges(Collection<Event> events, 
      Collection<String> attendees, int requestDuration) {
    // Sort time ranges in ascending order
    ArrayList<TimeRange> sortedBusyTimeRanges = 
        getSortedBusyTimeRanges(events, attendees);
    // If attendees are not in existing events, 
    // return whole day as a valid time range.
    ...
    ArrayList<TimeRange> mergedBusyTimeRanges = 
        getMergedBusyTimeRanges(sortedBusyTimeRanges);
    // Get available time ranges
    ArrayList<TimeRange> availableTimeRanges = 
        getAvailableTimeRanges(mergedBusyTimeRanges, requestDuration);
    return availableTimeRanges;
  }
  ...
  private ArrayList<TimeRange> 
    ...
    while ((i < mandatoryAvailableTimeRanges.size()) && 
      ...
      if (mandatoryTimeRange.overlaps(optionalTimeRange)) {
        TimeRange overlappingTimeRange = 
            getOverlappingTimeRange(mandatoryTimeRange, optionalTimeRange);
        // Check if the overlapping time range has a valid duration and 
        // add to mutual available time ranges.
